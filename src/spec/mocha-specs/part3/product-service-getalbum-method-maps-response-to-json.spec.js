...
describe("ProductService", () => {
  it("should map the result of get request to json with rxjs map function @product-service-getalbum-method-maps-response-to-json", () => {
    ...
    const importRxjs = tsquery(
      ast,
      "ImportDeclaration StringLiteral[value=rxjs/operators]" //rxjs/add/operator/map
    );

    assert(
      importRxjs.length > 0,
      "The ProductService isn't importing `map` from `rxjs/operators`." //rxjs/add/operator/map
    );
